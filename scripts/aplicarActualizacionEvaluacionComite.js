...
const rutaScript = resolve(directorio, '../basedatos/actualizar_evaluacion_quintiles_comite.sql');

try {
  const contenido = await readFile(rutaScript, 'utf8');
  const lotes = contenido.split(/^\s*GO\s*$/gim).map((lote) => lote.trim()).filter(Boolean);
  ...
  console.log(`Actualizacion de evaluacion y comite aplicada (${lotes.length} lotes).`);
} finally {
  await cerrarPool();
}
