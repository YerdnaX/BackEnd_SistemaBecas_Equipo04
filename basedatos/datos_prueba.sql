...
-- evaluacion). Este script agrega datos de prueba y aplica ajustes
-- idempotentes de Segmento 03 y 2FA para evitar correr scripts extra.
--
...
-- =====================================================================
-- 0. AJUSTES SEGMENTO 03 + 2FA (idempotentes)
-- =====================================================================
...
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Usuarios') AND name = 'RequiereDosFactores')
    ALTER TABLE dbo.Usuarios ADD RequiereDosFactores BIT NULL;
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Usuarios') AND name = 'RequiereDosFactores')
BEGIN
    UPDATE dbo.Usuarios SET RequiereDosFactores = 0 WHERE RequiereDosFactores IS NULL;
    ALTER TABLE dbo.Usuarios ALTER COLUMN RequiereDosFactores BIT NOT NULL;
END
GO
...
IF NOT EXISTS (
    ...
    WHERE dc.parent_object_id = OBJECT_ID('dbo.Usuarios') AND c.name = 'RequiereDosFactores'
)
    ALTER TABLE dbo.Usuarios ADD CONSTRAINT DF_Usuarios_RequiereDosFactores DEFAULT 1 FOR RequiereDosFactores;
GO
...
IF NOT EXISTS (SELECT 1 FROM dbo.PlantillasMensajes WHERE Codigo = 'APELACION_RESUELTA')
    ...
    ('APELACION_RESUELTA', 'Resultado de su apelacion',
     '<p>Su apelacion fue resuelta <strong>{{resultado}}</strong>.</p><p>{{motivo}}</p>');

IF NOT EXISTS (SELECT 1 FROM dbo.PlantillasMensajes WHERE Codigo = 'INVESTIGACION_ABIERTA')
    INSERT INTO dbo.PlantillasMensajes (Codigo, Asunto, Contenido) VALUES
    ('INVESTIGACION_ABIERTA', 'Se abrio un proceso de revision sobre su beca',
     '<p>Causal: {{causal}}.</p><p>Tiene {{plazoDias}} dias habiles para presentar sus descargos.</p>');

IF NOT EXISTS (SELECT 1 FROM dbo.PlantillasMensajes WHERE Codigo = 'INVESTIGACION_RESUELTA')
    INSERT INTO dbo.PlantillasMensajes (Codigo, Asunto, Contenido) VALUES
    ('INVESTIGACION_RESUELTA', 'Se resolvio el proceso de revision de su beca',
     '<p>Resultado: {{resultado}}.</p><p>{{motivo}}</p>');
GO

IF NOT EXISTS (SELECT 1 FROM dbo.PreguntasRespuestasChatbot WHERE Pregunta = N'¿Cómo apelo una resolución de beca?')
    INSERT INTO dbo.PreguntasRespuestasChatbot (Pregunta, Respuesta, Categoria, PalabrasClave) VALUES
    (N'¿Cómo apelo una resolución de beca?',
     N'Ingrese a su expediente, ubique la resolución y presione "Apelar". Tiene un plazo de días hábiles desde la notificación para hacerlo.',
     N'Apelaciones', N'apelar,apelacion,resolucion,inconforme');

IF NOT EXISTS (SELECT 1 FROM dbo.PreguntasRespuestasChatbot WHERE Pregunta = N'¿Qué pasa si me suspenden la beca?')
    INSERT INTO dbo.PreguntasRespuestasChatbot (Pregunta, Respuesta, Categoria, PalabrasClave) VALUES
    (N'¿Qué pasa si me suspenden la beca?',
     N'Recibirá una notificación con la causal y podrá presentar descargos dentro del plazo indicado antes de que se resuelva el caso.',
     N'Suspensión', N'suspension,suspendida,descargos,investigacion');
GO
...
-- Usuarios de prueba sembrados: no requieren doble factor en inicio de sesión.
IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Usuarios') AND name = 'RequiereDosFactores')
BEGIN
    UPDATE dbo.Usuarios
    SET RequiereDosFactores = 0
    WHERE Correo IN (
        N'admin.pruebas@cuc.ac.cr',
        N'coordinador.pruebas@cuc.ac.cr',
        N'trabajosocial.pruebas@cuc.ac.cr',
        N'comite.pruebas@cuc.ac.cr',
        N'aspirante1.pruebas@cuc.ac.cr',
        N'aspirante2.pruebas@cuc.ac.cr',
        N'aspirante3.pruebas@cuc.ac.cr',
        N'aspirante4.pruebas@cuc.ac.cr'
    );
END
GO
